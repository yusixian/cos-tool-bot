...
import { BACKUP_FILE_PREFIX, ENABLE_S3_BACKUP, S3_BUCKET_NAME, S3_ENDPOINT, S3_PUBLIC_URL, S3_REGION } from '@/constants';
...
export async function backupDBToS3IfEnabled(ctx: CommandContext<WrapperContext>) {
  // 上传数据库文件到 S3 备份
  if (ENABLE_S3_BACKUP) {
    try {
      const dbFile = fs.readFileSync('prisma/data.db');
      const key = BACKUP_FILE_PREFIX + 'data.db';
      await uploadS3(dbFile, key);
      await ctx.reply(`数据库备份已上传至 [S3 ☁️](${S3_PUBLIC_URL}/${key})`, { parse_mode: 'MarkdownV2' });
    } catch (error: any) {
